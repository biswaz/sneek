...
### GUI gaps
- **No variables/secrets editor in the GUI.** The command editor form has fields for General, Command Template, Access Control, SSH Tunnel, and MCP — but no key-value editor for `variables` and `secrets`. You have to edit the JSON directly for those. This is the highest-priority GUI gap.
- **No auto-fill for known command types.** The spec says the GUI should auto-fill setup commands and blocked patterns when you select a known type (Postgres, MySQL, Redis) and toggle read-only. Not implemented.
- **No tunnel status in GUI.** The menubar popover shows badges but doesn't poll the daemon for live tunnel status.

### Daemon gaps
- **No launchd plist / auto-start.** The daemon runs in the foreground (`sneekd start` blocks). Needs a `sneekd install` command that creates `~/Library/LaunchAgents/com.sneek.daemon.plist` for auto-start on login.
- **No auto-reconnect loop for tunnels.** The `SSHTunnelManager` detects dead tunnels and can re-spawn, but there's no background polling loop that proactively reconnects. Currently tunnels reconnect on next use.
- **No PID file.** `sneekd start` doesn't write a PID file, so `sneekd stop` relies on IPC (daemon must be running to receive stop command).

### Testing gaps
- **SSH tunnel integration test skipped** unless Remote Login is enabled in System Settings. The test exists but prints "(skipped)" if `ssh localhost` fails.
- **No real 1Password/Bitwarden tests.** These are behind the `SecretProvider` protocol and tested with mocks. Real CLI integration needs auth sessions.
- **No GUI tests.** SwiftUI UI testing is fragile — all GUI testing is manual.
